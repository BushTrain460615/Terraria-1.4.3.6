      ...
      if (Main.tileGlowMask[(int) drawData.tileCache.type] != (short) -1)
      {
        ...
        switch (drawData.tileCache.type)
        {
          case 129:
            ...
            drawData.drawTexture = this.GetTileDrawTexture(drawData.tileCache, tileX, tileY);
            color = Main.hslToRgb((float) (0.699999988079071 + Math.Sin(6.28318548202515 * (double) Main.GlobalTimeWrappedHourly * 0.159999996423721 + (double) tileX * 0.300000011920929 + (double) tileY * 0.699999988079071) * 0.159999996423721), 1f, 0.5f);
            color.A /= (byte) 2;
            color *= 0.3f;
            int num4 = 72;
...
label_25:
    ...
    private void EmitTreeLeaves(int tilePosX, int tilePosY, int grassPosX, int grassPosY)
    {
      ...
      if (WorldGen.SolidTile(vector2.ToTileCoordinates()))
        return;
      Gore.NewGoreDirect(vector2, Utils.RandomVector2(Main.rand, (float) -max, (float) max), passStyle, (float) (0.699999988079071 + (double) Main.rand.NextFloat() * 0.600000023841858)).Frame.CurrentColumn = Main.tile[tilePosX, tilePosY].color();
    }
      ...
      for (int index2 = 0; index2 < num1; ++index2)
      {
        ...
        if (tile != null && tile.active())
        {
          ...
          for (float num5 = 0.0f; (double) num5 < 1.0; num5 += 0.1666667f)
            Main.spriteBatch.Draw(texture2D, vector2_2 - Main.screenPosition + (6.283185f * num5).ToRotationVector2() * (float) (6.0 + (double) num3 * 2.0), new Rectangle?(r), color2, 0.0f, origin, 1f, effects, 0.0f);
        }
      }
    }
