...
namespace Terraria.GameContent
{
  public class PortalHelper
  {
      ...
      Vector2 theCrashVelocity)
    {
      ...
      for (int index = 0; index < (flag ? 2 : PortalHelper.EDGES.Length); ++index)
      {
        ...
        if ((double) Vector2.Dot(PortalHelper.EDGES[index], vector2_1) > 0.0 && PortalHelper.FindValidLine(tileCoordinates, (int) PortalHelper.EDGES[index].Y, (int) -(double) PortalHelper.EDGES[index].X, out bestPosition))
        {
          ...
          return PortalHelper.AddPortal(theBolt, position - PortalHelper.EDGES[index] * (flag ? 0.0f : 8f), (float) (Math.Atan2((double) PortalHelper.EDGES[index].Y, (double) PortalHelper.EDGES[index].X) + 1.57079637050629), (int) theBolt.ai[0], theBolt.direction);
        }
      }
      if (num != 0)
      {
        ...
        {
          position = new Vector2((float) (bestPosition.X * 16 + 8), (float) (bestPosition.Y * 16 + 8));
          return PortalHelper.AddPortal(theBolt, position, (float) Math.Atan2((double) vector2_2.Y, (double) vector2_2.X) - 1.570796f, (int) theBolt.ai[0], theBolt.direction);
        }
      }
      return -1;
    }
    ...
    private static bool CanPlacePortalOn(Tile t) => PortalHelper.DoesTileTypeSupportPortals(t.type) && WorldGen.SolidOrSlopedTile(t);
