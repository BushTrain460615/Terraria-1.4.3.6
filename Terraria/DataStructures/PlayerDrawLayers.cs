...
namespace Terraria.DataStructures
{
  public static class PlayerDrawLayers
  {
      ...
      else if (drawinfo.drawPlayer.wings == 40)
      {
        ...
        for (int index = 0; index < 1; ++index)
        {
          ...
          for (int frameY = num3; frameY < num4; ++frameY)
          {
            ...
            Vector2 vector2_10 = new Vector2(0.0f, 0.5f).RotatedBy(((double) drawinfo.drawPlayer.miscCounterNormalized * (2.0 + (double) num6) + (double) num6 * 0.5 + (double) index * 1.29999995231628) * 6.28318548202515) * (float) (num6 + 1);
      ...
      else
      {
        ...
        else if (drawinfo.drawPlayer.wings == 38)
        {
          ...
          for (int index = drawinfo.drawPlayer.shadowPos.Length - 2; index >= 0; --index)
          {
            ...
            {
              Vector2 vector2_15 = new Vector2(2f, 0.0f).RotatedBy((double) num10 / 0.0399999991059303 * 6.28318548202515);
        ...
        else if (drawinfo.drawPlayer.wings == 31)
        {
          ...
          Color color8 = Color.Lerp(Color.HotPink, Color.Crimson, (float) (Math.Cos(6.28318548202515 * ((double) drawinfo.drawPlayer.miscCounter / 100.0)) * 0.400000005960464 + 0.5)) with
          {
            A = 0
          };
          for (int index = 0; index < 4; ++index)
    ...
    public static void DrawPlayer_27_HeldItem(ref PlayerDrawSet drawinfo)
    {
      ...
      switch (index1)
      {
        ...
        case 3779:
          ...
          for (float num6 = 0.0f; (double) num6 < 4.0; ++num6)
          {
            drawData2 = new DrawData(TextureAssets.GlowMask[218].Value, (drawinfo.ItemLocation - Main.screenPosition + vector2_5).Floor() + (num6 * 1.570796f).ToRotationVector2() * num5, new Rectangle?(r2), color2, drawinfo.drawPlayer.itemRotation, origin3, adjustedItemScale, drawinfo.itemEffect, 0);
            drawinfo.DrawDataCache.Add(drawData2);
          }
        ...
        default:
          if (heldItem.useStyle == 5)
          {
            ...
            if (index1 != 3788)
              break;
            float num10 = (float) ((double) ((float) ((double) drawinfo.drawPlayer.miscCounter / 75.0 * 6.28318548202515)).ToRotationVector2().X * 1.0 + 0.0);
            Color color3 = new Color(80, 40, 252, 0) * (float) ((double) num10 / 2.0 * 0.300000011920929 + 0.850000023841858) * 0.5f;
            for (float num11 = 0.0f; (double) num11 < 4.0; ++num11)
            {
              drawData6 = new DrawData(TextureAssets.GlowMask[220].Value, new Vector2((float) (int) ((double) drawinfo.ItemLocation.X - (double) Main.screenPosition.X + (double) vector2_11.X), (float) (int) ((double) drawinfo.ItemLocation.Y - (double) Main.screenPosition.Y + (double) vector2_11.Y)) + (num11 * 1.570796f + drawinfo.drawPlayer.itemRotation).ToRotationVector2() * num10, new Rectangle?(), color3, drawinfo.drawPlayer.itemRotation, origin7, adjustedItemScale, drawinfo.itemEffect, 0);
              drawinfo.DrawDataCache.Add(drawData6);
            }
            break;
          }
    ...
    public static void DrawPlayer_38_EyebrellaCloud(ref PlayerDrawSet drawinfo)
    {
      ...
      for (int index = 0; index < 2; ++index)
      {
        Vector2 vector2_2 = new Vector2(index == 0 ? -num4 : num4, 0.0f).RotatedBy((double) num3 * 6.28318548202515 * (index == 0 ? 1.0 : -1.0));
