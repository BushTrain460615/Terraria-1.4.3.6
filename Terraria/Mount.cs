...
namespace Terraria
{
  public class Mount
  {
    ...
    public IEntitySource GetProjectileSpawnSource(Player mountedPlayer) => (IEntitySource) new EntitySource_Mount((Entity) mountedPlayer, this.Type);
      ...
      float shadow)
    {
      ...
      switch (this._type)
      {
        ...
        case 23:
          ...
          {
            drawData = new DrawData(texture2D1, (Position + ((float) index * 1.570796f).ToRotationVector2() * num6).Floor(), new Rectangle?(rectangle1), color5, rotation3, origin, scale1, effect, 0);
            drawData.shader = Mount.currentShader;
            playerDrawData.Add(drawData);
          }
          break;
